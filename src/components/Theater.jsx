...
//
//  New:
//    • FloatingMark button — lightweight "mark this moment" overlay
//    • CharacterLayer passes beatId + dispute props to ThoughtBubble
//    • sceneType switches between RoomBackground / OutdoorBackground
// ─────────────────────────────────────────────────────────────
    ...
    <div className="absolute inset-0 overflow-hidden">
      <div className="absolute top-0 left-0 right-0 room-ceiling" style={{ height: '70px' }}>
        <div className="absolute" style={{ left: '50%', top: '16px', transform: 'translateX(-50%)' }}>
          ...
          <div style={{ width: '26px', height: '18px', margin: '0 auto',
      ...
      <div className="absolute left-0 right-0 room-wall" style={{ top: '70px', bottom: '98px' }}>
        ...
        <div className="absolute" style={{ left: '8%', top: '28px', width: '108px', height: '128px',
        ...
        {[
          ...
          <div key={i} className="absolute" style={{ left:f.left,right:f.right,top:f.top,
            width:f.w,height:f.h,background:'#5a3a1a',padding:'5px',
            boxShadow:'2px 2px 8px rgba(0,0,0,0.4)' }}>
            <div style={{ width:'100%',height:'100%',background:f.c,opacity:0.6 }} />
          </div>
        ))}
        <div className="absolute" style={{ left:'37%',bottom:'18px',width:'175px',height:'50px',
          ...
          <div style={{ position:'absolute',top:'7px',right:'14px',width:'20px',height:'20px',
        ...
        <div className="absolute" style={{ right:'6%',bottom:'5px',width:'196px',height:'88px' }}>
          <div style={{ position:'absolute',top:0,left:0,right:0,height:'54px',
          ...
          <div style={{ position:'absolute',bottom:'15px',left:'-7px',right:'-7px',height:'35px',
            background:'linear-gradient(180deg,#4a3860,#382848)',borderRadius:'3px',boxShadow:'0 4px 8px rgba(0,0,0,0.5)' }} />
        ...
        <div className="absolute" style={{ right:'29%',bottom:'3px' }}>
          ...
          <div style={{ width:'26px',height:'16px',marginTop:'-3px',marginLeft:'-11px',
...
function OutdoorBackground() {
  ...
  return (
    <div className="absolute inset-0 overflow-hidden">
      ...
      <div className="absolute" style={{ right:'14%',top:'10%',width:'36px',height:'36px',borderRadius:'50%',
      ...
      {stars.map(([x,y],i) => (
        <div key={i} className="absolute" style={{ left:`${x}%`,top:`${y}%`,width:'2px',height:'2px',
          borderRadius:'50%',background:'rgba(255,255,255,0.75)',boxShadow:'0 0 4px rgba(255,255,255,0.4)',
          animation:`pulseSoft ${1.8+i*0.25}s ease-in-out infinite alternate` }} />
      ))}
      {[{l:'3%',w:'45px',h:'55px'},{l:'10%',w:'28px',h:'38px'},{l:'76%',w:'52px',h:'48px'},
        {l:'86%',w:'32px',h:'32px'},{l:'92%',w:'22px',h:'42px'}].map((b,i) => (
        <div key={i} className="absolute" style={{ left:b.l,bottom:'98px',width:b.w,height:b.h,background:'#06080f' }}>
          <div style={{ position:'absolute',top:'10px',left:'6px',width:'5px',height:'5px',
            background:'rgba(255,220,100,0.5)',boxShadow:'0 0 4px rgba(255,200,80,0.3)' }} />
        </div>
      ))}
      <div className="absolute" style={{ left:0,right:0,bottom:'98px',height:'48px',
        background:'linear-gradient(180deg,#101828,#0c1220)',borderTop:'1px solid rgba(255,255,255,0.035)' }} />
      ...
      <div className="absolute" style={{ right:'17%',bottom:'120px' }}>
        ...
        <div style={{ display:'flex',justifyContent:'space-between',padding:'0 8px' }}>
          {[0,1,2].map(k => <div key={k} style={{ width:'4px',height:'20px',background:'#3a2010' }} />)}
        </div>
      </div>
      <div className="absolute" style={{ left:'12%',bottom:'98px' }}>
        ...
        <div style={{ position:'absolute',top:'-14px',right:'-34px',width:'14px',height:'10px',
          background:'linear-gradient(180deg,#fffbe0,#f0d070)',
          boxShadow:'0 0 22px rgba(240,200,80,0.55),0 0 60px rgba(240,200,80,0.12)',borderRadius:'2px' }} />
...
function FloatingMark({ onMark, phase }) {
  ...
  return (
    <div className="absolute bottom-4 right-4 z-25 flex flex-col items-end gap-1.5">
      {/* Emoji picker */}
      {open && (
        <div className="flex gap-1.5 anim-fadeIn">
          {MARK_EMOJIS.map(e => (
            <button key={e}
              onClick={() => { onMark(e); setOpen(false) }}
              className="w-8 h-8 flex items-center justify-center rounded-lg text-[14px] transition-all hover:scale-110"
      ...
      <button
        onClick={() => setOpen(o => !o)}
        className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg font-mono text-[9px] transition-all"
        style={{
          background:  open ? 'rgba(255,255,255,0.12)' : 'rgba(0,0,0,0.55)',
...
export default function Theater({
  beat, personas, showThoughts, sceneType = 'enclosed', phase,
  ...
  return (
    <div className="relative w-full h-full flex flex-col">
      ...
      {/* Subtitle */}
      <div style={{ minHeight:'52px',flexShrink:0,
