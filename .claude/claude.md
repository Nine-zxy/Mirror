...
### 1. Together 模式双设备同步 bug 修复 ✅ Done
- [x] 用户反馈：加入房间后没有同步
- [x] 排查 WebSocket sync server（端口 3001）连接逻辑
- [x] 房间码已改为 4 位（确认实现）
- [x] 测试：两个浏览器标签页完整走通 Together 流程
- [x] 测试：局域网两台设备走通（`http://局域网IP:5173`）
- 验收标准：两台设备从 intro → self_confirm → solo_viewing → together_viewing → divergence 全流程无报错

### 2. ConflictInput 页面字段重构 ✅ Done
- [x] 聊天记录：只需一方上传，上传后增加"你是对话中的哪一位？"
- [x] Together 模式：上传方的记录通过 sync 共享给对方
- [x] 关切字段改为：① "你最想让对方理解的是什么？" ② "你当时最强烈的感受是什么？"
- [x] "前置情境（可选）" → 改为必填 "一句话背景"
- [x] 同步提示语升级："对方此刻正在独立完成同样的描述…"
- [x] 保留沟通风格投票（每人只选自己的）
- 参考：会议记录 3.18 讨论内容 + 防遗漏check文件

### 3. 时间轴改平 ✅ Done
- [x] ConflictTimeline 去掉坡度/gradient，改为平直
- [x] 苏老师+Stephy 都确认不需要坡度
- 参考：会议记录 3.18
